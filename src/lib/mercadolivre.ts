...
// Helper: parse JSON seguro - retorna o JSON ou lanca erro descritivo
async function safeJsonParse(response: Response, context: string): Promise<Record<string, unknown>> {
...
export async function exchangeCodeForToken(code: string) {
      ...
      return safeJsonParse(response, "exchangeCodeForToken");
}
...
export async function refreshAccessToken(refreshToken: string) {
      ...
      return safeJsonParse(response, "refreshAccessToken");
}

// Faz chamada autenticada a API do ML com retry em caso de token expirado
export async function mlApiCall(endpoint: string, accessToken: string) {
  ...
  return safeJsonParse(response, `mlApiCall(${endpoint})`);
}
