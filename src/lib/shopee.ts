...
// Remove prefixo "shpk" da chave se presente (a chave real e o que vem apos o prefixo)
function getRawPartnerKey(key: string): string {
...
function generateSign(partnerKey: string, ...args: (string | number)[]): string {
    const rawKey = getRawPartnerKey(partnerKey);
    const baseString = args.join("");
    return crypto.createHmac("sha256", rawKey).update(baseString).digest("hex");
}
...
export async function getShopeeOrderDetails(
    ...
    const params: Record<string, string | number> = {
          ...
          response_optional_fields:
                  "buyer_user_id,buyer_username,estimated_shipping_fee,actual_shipping_fee,item_list,pay_time,buyer_cancel_reason,cancel_by,package_list,shipping_carrier,payment_method,total_amount,order_income",
    };
