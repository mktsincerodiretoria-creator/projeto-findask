...
export default function AmazonPage() {
  ...
  const fetchData = useCallback(async (from?: string, to?: string) => {
    ...
    try {
      ...
      const amzAccounts = Array.isArray(accountsData) ? accountsData.filter((a: { platform: string }) => a.platform === "AMAZON") : [];
      setHasAccount(amzAccounts.length > 0);
  ...
  async function handleSync() {
    try {
      await fetch("/api/sync/amazon", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {hasAccount && (
          <button onClick={handleSync} className="px-4 py-2 rounded-lg text-sm font-medium text-white bg-amber-600 hover:bg-amber-700">
            Sincronizar Amazon
          </button>
        )}
      </div>
      ...
      {!hasAccount && hasAccount !== null && orders.length === 0 ? (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-6 text-center">
          <p className="text-amber-800 text-lg font-semibold mb-2">Conecte sua Amazon</p>
          <p className="text-amber-600 text-sm mb-4">Conecte via API ou importe a planilha abaixo.</p>
          <a href="/contas" className="px-6 py-2 bg-amber-600 text-white rounded-lg font-medium hover:bg-amber-700">Ir para Contas</a>
        </div>
      ) : loading ? (
        <div className="flex items-center justify-center h-64">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-amber-600" />
          <span className="ml-3 text-gray-500">Carregando dados da Amazon...</span>
        </div>
      ) : (
        <>
          ...
          <div className="bg-white rounded-lg border p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <MetricCard title="(-) Taxa Amazon" value={totals.fee} icon="💳" />
              <MetricCard title="(-) Frete/FBA" value={totals.freteVend} icon="🚚" />
      ...
      <ImportPlanilha platform="AMAZON" onImportComplete={() => fetchData(dateRange.from, dateRange.to)} />

      <div className="border-t-2 border-amber-400 pt-6">
        <h2 className="text-xl font-bold text-gray-900 mb-1">Vendas - Amazon</h2>
          ...
          <table className="w-full text-sm min-w-[1400px]">
            <thead className="bg-gray-50 sticky top-0">
              <tr>
                ...
                <SortHeader label="Taxa (-)" field="fee" sortKey={sortKey} sortDir={sortDir} onSort={handleSort} />
                <SortHeader label="Frete/FBA (-)" field="freteVend" sortKey={sortKey} sortDir={sortDir} onSort={handleSort} />
            ...
            <tbody>
              ...
              {rows.length === 0 && (
                <tr><td colSpan={14} className="px-3 py-8 text-center text-gray-500">Nenhuma venda da Amazon. Conecte ou importe planilha acima.</td></tr>
              )}
            </tbody>
          </table>
        </div>
