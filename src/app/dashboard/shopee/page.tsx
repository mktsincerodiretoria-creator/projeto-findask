...
export default function ShopeePage() {
  ...
  const fetchData = useCallback(async (from?: string, to?: string) => {
    ...
    try {
      ...
      const adsParams = new URLSearchParams({ platform: "SHOPEE" });
      ...
      const shopeeAccounts = Array.isArray(accountsData) ? accountsData.filter((a: { platform: string }) => a.platform === "SHOPEE") : [];
      setHasAccount(shopeeAccounts.length > 0);
    ...
    <div className="space-y-6">
      ...
      {!hasAccount && hasAccount !== null ? (
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-6 text-center">
          <p className="text-orange-800 text-lg font-semibold mb-2">Conecte sua Shopee</p>
          <p className="text-orange-600 text-sm mb-4">Para ver as vendas da Shopee, conecte sua conta em Contas.</p>
          <a href="/contas" className="px-6 py-2 bg-orange-500 text-white rounded-lg font-medium hover:bg-orange-600">Ir para Contas</a>
        </div>
      ) : loading ? (
        <div className="flex items-center justify-center h-64">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-orange-500" />
          <span className="ml-3 text-gray-500">Carregando dados da Shopee...</span>
        </div>
      ) : (
        <>
          {/* Dashboard */}
          ...
          <div className="bg-white rounded-lg border p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <MetricCard title="(-) Tarifa Shopee" value={totals.fee} icon="💳" />
          ...
          {/* ADS */}
          <AdsSection platform="SHOPEE" platformLabel="Shopee" borderColor="border-orange-400" totals={adsTotals} campaigns={adsCampaigns} onImportComplete={() => fetchData(dateRange.from, dateRange.to)} />
          ...
          <div className="border-t-2 border-orange-400 pt-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-6 gap-3 mb-4">
              ...
              <div className="bg-white rounded-lg border p-3"><p className="text-xs text-gray-500">(-) Custo</p><p className="text-lg font-bold text-red-600">{formatCurrency(totals.cost)}</p></div>
              <div className="bg-white rounded-lg border p-3"><p className="text-xs text-gray-500">(-) Imposto</p><p className="text-lg font-bold text-red-600">{formatCurrency(totals.tax)}</p></div>
              <div className="bg-white rounded-lg border p-3"><p className="text-xs text-gray-500">(-) Tarifa</p><p className="text-lg font-bold text-red-600">{formatCurrency(totals.fee)}</p></div>
              <div className="bg-white rounded-lg border p-3"><p className="text-xs text-gray-500">(-) Frete Vend</p><p className="text-lg font-bold text-red-600">{formatCurrency(totals.freteVend)}</p></div>
              <div className="bg-white rounded-lg border p-3"><p className="text-xs text-gray-500">= Margem</p><p className={`text-lg font-bold ${totals.margin >= 0 ? "text-green-600" : "text-red-600"}`}>{formatCurrency(totals.margin)}</p></div>
            </div>
            ...
            <div className="bg-white rounded-lg border overflow-x-auto">
              <table className="w-full text-sm min-w-[1400px]">
                ...
                <tbody>
                  ...
                  {rows.length === 0 && (
                    <tr><td colSpan={14} className="px-3 py-8 text-center text-gray-500">Nenhuma venda da Shopee no periodo. Conecte e sincronize em Contas.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
