...
async function processOrder(
  ...
  try {
      ...
      const totalShipping = Number(fullOrder.shipping?.cost || 0);
        debugInfo.shippingFromOrder = {
                cost: fullOrder.shipping?.cost,
  ...
  const savedOrder = await prisma.order.upsert({
        ...
        update: {
                ...
                paidDate: mlOrder.date_closed
                  ? new Date(mlOrder.date_closed as string)
                          : null,
        },
        create: {
                accountId,
    ...
    try {
      ...
      if (accounts.length === 0) {
              return NextResponse.json(
                { error: "Nenhuma conta ML ativa encontrada. Configure em Contas > Mercado Livre." },
                { status: 404 }
                      );
      }
      ...
      for (const account of accounts) {
            ...
            try {
                ...
                let accessToken = account.accessToken;
                      if (account.refreshToken) {
                                  try {
                                                ...
                                                // Se refresh falhar, registra o erro mas tenta com o token atual
                                    console.warn(`Aviso: falha ao renovar token para conta ${account.id}:`, refreshError);
                                  }
                      }
                ...
                // Limite de seguranca: max 300 pedidos por sync para nao estourar timeout
