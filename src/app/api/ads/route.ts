...
export async function GET(request: NextRequest) {
          try {
            ...
            const totals = metrics.reduce(
                          (acc, m) => ({ impressions: acc.impressions + m.impressions, clicks: acc.clicks + m.clicks, spend: acc.spend + m.spend, revenue: acc.revenue + m.revenue, orders: acc.orders + m.orders }),
            ...
            const totalRevenue = await prisma.order.aggregate({ where: orderWhere, _sum: { totalAmount: true } });
            ...
            const byCampaign: Record<string, { campaignName: string; platform: string; campaignId: string; spend: number; revenue: number; clicks: number; impressions: number; orders: number }> = {};
                      for (const m of metrics) {
                                    ...
                                    byCampaign[key].spend += m.spend; byCampaign[key].revenue += m.revenue; byCampaign[key].clicks += m.clicks; byCampaign[key].impressions += m.impressions; byCampaign[key].orders += m.orders;
                      }
            ...
            const bySku: Record<string, { sku: string; spend: number; revenue: number; clicks: number; orders: number }> = {};
                      for (const m of metrics) {
                                    ...
                                    bySku[m.sku].spend += m.spend; bySku[m.sku].revenue += m.revenue; bySku[m.sku].clicks += m.clicks; bySku[m.sku].orders += m.orders;
                      }
            ...
            const dailyMap: Record<string, { date: string; spend: number; revenue: number; clicks: number; orders: number }> = {};
                      for (const m of metrics) {
                                    ...
                                    dailyMap[dk].spend += m.spend; dailyMap[dk].revenue += m.revenue; dailyMap[dk].clicks += m.clicks; dailyMap[dk].orders += m.orders;
                      }

            return NextResponse.json({ totals: { ...totals, cpc, ctr, acos, roas, tacos, totalFaturamento }, byCampaign: Object.values(byCampaign), bySku: Object.values(bySku), daily: Object.values(dailyMap).sort((a, b) => a.date.localeCompare(b.date)) });
          } catch (error) {
...
async function getToken() {
          ...
          if (account.tokenExpires && account.tokenExpires < new Date()) {
                      ...
                      await prisma.account.update({ where: { id: account.id }, data: { accessToken: nt.access_token, refreshToken: nt.refresh_token, tokenExpires: new Date(Date.now() + nt.expires_in * 1000) } });
          }
...
async function syncMLAds() {
  ...
  // Diagnostico: testa varios endpoints para descobrir qual funciona
  const [r1, r2, r3, r4, r5] = await Promise.all([
              rawCall(`${BASE}/marketplace/advertising/MLB/advertisers/${id}/product_ads/campaigns/search`, token, "2"),
              rawCall(`${BASE}/marketplace/advertising/MLB/advertisers/${id}/product_ads/campaigns/search`, token),
              rawCall(`${BASE}/users/${id}/advertising/product_ads/v2/campaigns`, token),
              rawCall(`${BASE}/advertising/advertisers?user_id=${id}`, token),
  ...
  const diag: any = {
              ...
              users_me: { status: r5.status, id: r5.body?.id, nickname: r5.body?.nickname },
              advertising_advertisers: { status: r4.status, body: r4.body },
              campaigns_v2_api_version: { status: r1.status, body: r1.body },
              campaigns_sem_apiversion: { status: r2.status, body: r2.body },
              campaigns_users_endpoint: { status: r3.status, body: r3.body },
  };

  // Tenta cada resultado em ordem de preferencia
  const results = [
  ...
  if (!campaignData) {
              // Nenhum endpoint funcionou — retorna diagnostico completo
