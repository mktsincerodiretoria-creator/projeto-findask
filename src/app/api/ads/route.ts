...
async function mlAdsApiCall(endpoint: string, accessToken: string) {
      ...
      if (!response.ok) {
              ...
              // Detecta erro 401 de usuario nao autorizado para Ads
        if (response.status === 401) {
                  try {
                              const errJson = JSON.parse(errorText);
                              if (errJson.error === "user_not_authorized" || errJson.message?.includes("not authorized")) {
              ...
              throw new Error(`ML Ads API (${response.status}): ${errorText}`);
      }
      return response.json();
}
...
async function syncMLAds() {
  ...
  try {
          // Passo 1: Busca o advertiser_id correto
        ...
        for (const camp of campaigns) {
          ...
          // Se o usuario nao tem Product Ads habilitado no ML, retorna mensagem amigavel
        if (errMsg.includes("ADS_NOT_ENABLED")) {
                  return NextResponse.json({
                              status: "no_ads_account",
                              message: "Produto Ads nao habilitado. Para usar esta funcionalidade, acesse o Mercado Livre e ative os Anuncios de Produto (Product Ads) na sua conta de vendedor.",
                              totalSynced: 0,
                              debug: debugInfo,
                  });
        }
          return NextResponse.json({
                    error: "Erro ao acessar API de Ads: " + errMsg,
                    debug: debugInfo,
                    totalSynced,
          });
  }
