...
export async function GET(request: NextRequest) {
  try {
    ...
    // Busca metricas de ads e faturamento total em PARALELO
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    if (from || to) {
      ...
      if (to) {
        const toDate = new Date(to);
        toDate.setHours(23, 59, 59, 999);
        orderWhere.orderDate.lte = toDate;
      }
    }
    orderWhere.status = { notIn: ["cancelled", "returned", "refunded", "CANCELLED", "RETURNED", "IN_CANCEL"] };

    // Usa daily_metrics para TACOS em vez de aggregate pesado nas orders
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const metricsWhere: any = {};
    if (platform) metricsWhere.platform = platform;
    if (from || to) {
      metricsWhere.date = {};
      if (from) metricsWhere.date.gte = new Date(from);
      if (to) {
        const toDate = new Date(to);
        toDate.setHours(23, 59, 59, 999);
        metricsWhere.date.lte = toDate;
      }
    }
    ...
    // TACOS usa daily_metrics (tabela indexada e leve) em vez de orders (pesada)
    const totalFaturamento = dailyMetrics._sum.revenue || 0;
...
async function syncMLAds() {
  ...
  if (!campaignData) {
    return NextResponse.json({ status: "erro_api_ads", message: "Nenhum endpoint de campanhas retornou dados" });
  }
  ...
  return NextResponse.json({ status: "success", totalSynced, campanhas: campaigns.length, usedEndpoint });
}
